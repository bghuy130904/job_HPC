...
# Double hybrid: xc cho phan DFT + he so cho phan MP2.
# 'cmp2' la he so chung; neu co 'css'/'cos' thi dung spin-component scaling.
# Kiem chung: bai bao dung orbital UNRESTRICTED cho DH tren duong phan ly
# (khac phan can bang), vi tham chieu restricted cho mu PHAN KY khi r -> vo cung.
DH_FUNCS = {
    "b2plyp":   {"xc": "0.53*HF + 0.47*B88, 0.73*LYP",     "cmp2": 0.27},
    "b2gpplyp": {"xc": "0.65*HF + 0.35*B88, 0.64*LYP",     "cmp2": 0.36},
    "pbe0dh":   {"xc": "0.5*HF + 0.5*PBE, 0.875*PBE",      "cmp2": 0.125},
    "pbeqidh":  {"xc": "0.693*HF + 0.307*PBE, 0.667*PBE",  "cmp2": 0.333},
    # hybrid thuong, de doi chung (mat do KS = mat do phuong phap -> giai tich)
    "pbe0":     {"xc": "PBE0",  "cmp2": 0.0},
    "b3lyp":    {"xc": "B3LYP", "cmp2": 0.0},
}
OBDH_THRESH  = 1e-8
OBDH_NITER   = 300
...
def _lazy_imports():
    global gto, scf, mp, dft, stabilize_scf, OBDH_CL, OBMP2_CL
    from pyscf import gto, scf, mp, dft                     # noqa: F401
...
def _solve_ks(mol, xc, dm0=None, hcore=None, stabilize=True):
    """Cung chuoi nhu _solve nhung cho Kohn-Sham."""
    mf = _mk_uks(mol, xc, hcore)
    mf.kernel(dm0=dm0 if dm0 is not None else _mixed_guess(mol))
    if not mf.converged:
        mf = mf.newton(); mf.kernel(mf.mo_coeff, mf.mo_occ)
    if stabilize:
        mf = stabilize_scf(mf, max_macro_cycles=10, verbose=False)
        try:
            if abs(mf.spin_square()[0]) < 1e-6:
                alt = _mk_uks(mol, xc, hcore)
...
def _dh_energy(mol, key, hcore=None, dm0=None):
    ...
    if "css" in cfg:                       # spin-component scaling
        ec = cfg["css"] * pt.e_corr_ss + cfg["cos"] * pt.e_corr_os
    else:
        ec = cfg["cmp2"] * pt.e_corr
    return float(mf.e_tot + ec), mf


def dipole_ff_dh(mol, key, mf0, e0):
    """Finite field cho double hybrid. Hybrid thuong (cmp2=0) lay giai tich
    vi mat do KS CHINH LA mat do cua phuong phap."""
    if DH_FUNCS[key]["cmp2"] == 0.0:
        return np.asarray(mf0.dip_moment(unit='Debye', verbose=0), float), 0.0
    h0 = _mk_uks(mol, DH_FUNCS[key]["xc"]).get_hcore()
    ...
    for x in range(3):
        ...
        ep, _ = _dh_energy(mol, key, h0 + pert, dm0)
        em, _ = _dh_energy(mol, key, h0 - pert, dm0)
        d[x] = (ep - em) / (2 * FIELD)
...
def _zero_field(mol, key, mf):
    ...
    if key in DH_FUNCS:
        e, mfk = _dh_energy(mol, key)
        v = mfk.dip_moment(unit='Debye', verbose=0)
        return float(e), np.asarray(v, float), mfk       # mfk dung lam mf0 cho FF
...
def dipole_ff(mol, key, mf0, e0, mo_start=None):
    if key in DH_FUNCS:
        return dipole_ff_dh(mol, key, mf0, e0)
...
METHODS = {"uhf": "UHF", "ump2": "UMP2", "obmp2": "OBMP2", "obdh": "OBDH",
           "b2plyp": "B2PLYP", "b2gpplyp": "B2GPPLYP", "pbe0dh": "PBE0DH",
           "pbeqidh": "PBEQIDH", "pbe0": "PBE0", "b3lyp": "B3LYP"}
ANALYTIC = {"uhf", "pbe0", "b3lyp"}      # bien phan -> mat do cho dipole dung
