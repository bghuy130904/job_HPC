...
REF="/home/giahuy/Code/job/CCSD/bench_dip_noneq_config/geometry/dissociation_reference.json"
OUTDIR="/data/giahuy/Result/CCSD/bench_dip_noneq_config"
...
python "$CODE" --input "$INPUT" --outdir "$OUTDIR" --system "$S" \
       --ref-json "$REF" --methods ccsd ccsd_t \
    | grep -E "^\[|^  r=|MERGE|===|branch_switch|hysteresis|method|benchmark|CCSD|CCSD_T"
