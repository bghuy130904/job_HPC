#!/usr/bin/env python3
"""
Dipole moment doc duong phan ly FH va FCl — CCSD va CCSD(T).

VI SAO CHAY CA HAI
    Bai bao bench ba muc CC (CCSD, CCSD(T), CCSD(2)) truoc khi chon tham chieu,
    vi ket qua khong hien nhien: quanh diem Coulson-Fischer cua FH (r = 1.35 A),
    CCSD(T) co KINK tai cho chuyen tiep phan cuc spin, con CCSD va CCSD(2) thi
    muot. Nen ho dung CCSD(2) lam chuan cho FH. Voi FCl thi CCSD(T) va CCSD(2)
    trung khit -> CCSD(T) dung duoc.
    Chay CCSD + CCSD(T) o day la tai hien phep kiem tra do: neu ban thay CCSD(T)
    gay o FH ma khong gay o FCl, ban da xac nhan lua chon cua ho bang so lieu
    cua chinh minh. CCSD(2) khong co trong pyscf nen khong tinh duoc.
...
def _lazy_imports():
    global gto, scf, mp, cc, stabilize_scf, OBDH_CL, OBMP2_CL
    from pyscf import gto, scf, mp, cc                      # noqa: F401
...
_CC_CACHE = {}          # (id(mol), bytes(hcore)) -> (E_CCSD, E_CCSD(T))


def _cc_energies(mol, hcore, dm0):
    """Tra ve (E_CCSD, E_CCSD(T)) trong MOT lan chay. Hai muc dung chung
    nghiem CCSD nen khong tinh lai — tiet kiem mot nua chi phi."""
    kk = (id(mol), hcore.tobytes() if hcore is not None else b'0')
    if kk in _CC_CACHE:
        return _CC_CACHE[kk]
    mf = _solve(mol, dm0=dm0, hcore=hcore, stabilize=True)
    mycc = cc.CCSD(mf)
    mycc.verbose = 0
    mycc.kernel()
    e_ccsd = float(mycc.e_tot)
    try:
        e_t = e_ccsd + float(mycc.ccsd_t())
    except Exception:
        e_t = float('nan')
    _CC_CACHE[kk] = (e_ccsd, e_t)
    if len(_CC_CACHE) > 64:                 # chi giu quanh diem r hien tai
        _CC_CACHE.pop(next(iter(_CC_CACHE)))
    return e_ccsd, e_t


def _energy(mol, key, hcore, dm0, mo_start=None):
    if key in ("ccsd", "ccsd_t"):
        e_c, e_t = _cc_energies(mol, hcore, dm0)
        return e_c if key == "ccsd" else e_t
...
def _zero_field(mol, key, mf):
    ...
    if key in ("ccsd", "ccsd_t"):
        e_c, e_t = _cc_energies(mol, None, mf.make_rdm1())
        return (e_c if key == "ccsd" else e_t), np.zeros(3), None
    if key == "uhf":
    ...
    for s, g in df.groupby("system"):
        ...
        if 'benchmark' in g and g.benchmark.notna().any():
            ...
            print(f"    {'benchmark':8}{k.benchmark.abs().max():9.4f}{rb:9.3f}"
                  f"{'-':>9}{'-':>9}")
            for F in ["UHF", "UMP2", "OBMP2", "OBDH", "CCSD", "CCSD_T"]:
                c = f"mu_{F}"
